...
-   wieviel muss geschrieben werden? siehe benedikt :-)
...
all_accidents_gpkg <- st_read("data/roadtrafficaccidentlocations.gpkg")
bicycle_accidents_gpkg <- all_accidents_gpkg |>
  filter(AccidentInvolvingBicycle == "true")
```

```{r}
bike_acc_stat_zones <- st_join(bicycle_accidents_gpkg, stat_zones)
```
