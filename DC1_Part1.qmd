...
title(main = "k-Nearest Neighbours Plot for 2018")

# We can somehow see a knee at about 900 m.
abline(h = 900, col = "red")

# So, let's also try 400m.
...
title(main = "k-Nearest Neighbours Plot for 2019")

# We can somehow see a knee at about 800 m.
abline(h = 800, col = "red")

# So, let's also try  500 m.
graphics::abline(h = 400, col = "blue")
...
title(main = "k-Nearest Neighbours Plot for 2020")

# We can somehow see a knee at about 1000 m.
abline(h = 1000, col = "red")

# So, let's also try  400 m.
graphics::abline(h = 400, col = "blue")
...
title(main = "k-Nearest Neighbours Plot for 2021")
...
plot(bicycle_crd_2018, 
     ...
     main = "2018 DBSCAN result with eps = 200 m",
     asp = 1)
...
plot(bicycle_crd_2019, 
     ...
     main = "DBSCAN result with eps = 200 m",
     asp = 1)
...
plot(bicycle_crd_2020, 
     ...
     main = "DBSCAN result with eps = 250 m",
     asp = 1)

# 2021
# ------------------------------------------------------------------------------
# Now compute DBSCAN with different values for eps.
db21_03 <- dbscan::dbscan(bicycle_crd_2021, eps = 250, minPts = 3)
db21_03
...
```{r}
# First run with eps = 500 m and minPts = 3
...
plot(bicycle_crd_2018, 
     ...
     main = "OPTICS extracted clusters (eps = 500 m, minPts = 3) for 2018",
     asp = 1)

# Second run with eps = 150 m and minPts = 5
db <- dbscan::optics(bicycle_crd_2018, eps = 150, minPts = 5)
db
clusters <- dbscan::extractXi(db, xi = 0.05)  # Re-extract clusters for this run
plot(bicycle_crd_2018, 
     cex = 0.5, pch = 19, col = clusters$cluster + 1,
     main = "OPTICS extracted clusters (eps = 150 m, minPts = 5) for 2018",
     asp = 1)
...
## Task 9 Standard Deviational Ellipse:
...
ggplot() +
  ...
  geom_sf(data = bicycle_accidents_2019) +
...
sde_crd_21 <- phonTools::sdellipse(bicycle_crd_2021, stdev = 1, show = FALSE)

ggplot() +
  geom_polygon(aes(x = sde_crd_21[,1], y = sde_crd_21[,2]), 
               colour = "darkgreen", fill = "lightgreen", 
               linewidth = 0.8, alpha = 0.4) +
  geom_sf(data = bicycle_accidents_2021) +
  geom_point(aes(x = mean(bicycle_crd_2021[,1]), y = mean(sde_crd_21[,2])), 
                 colour = "red", size = 3) +
  coord_sf(datum = crs_lv95) +
  xlab("Easting [m]") + ylab("Northing [m]") +    # label axes
  ggtitle("Standard Deviational Ellipse for Bicycle Accidents 2021")
...
## Task 12 KDE using density() function from the stats package
...
mapview::mapview(zh_city_districts, alpha.regions = 0.3) +
...
Discuss your results. What did you find regarding the hot and cold spots in your accident count layers? How do they compare to each other across layers and across (past) methods?
etc. Discuss also the influence of the parameter settings (e.g., neighbor search distance, spatial
weight formation) on your results.
