...
bicycle_crd_ppp <- spatstat.geom::ppp(x = bicycle_crd[,1], y = bicycle_crd[,2], window = win_all, unitname = c("meter", "meters"))

# Compute K-function using Kest(), with edge correction.
bicycle_crd_kf <- spatstat.explore::Kest(bicycle_crd_ppp, correction = "Ripley")
...
plot(bicycle_crd_kf, main = "K-function for all bicycle accidents points")
```
...
#| 
# Compute K-function using Kest(), with edge correction.
bicycle_crd18_kf <- spatstat.explore::Kest(bicycle_crd18_ppp, correction = "Ripley")

# When plotting the result of K-, L-, G-functions, spatstat uses an S3 method plot.fv()
# that overwrites the basic plot() function and specializes it.
plot(bicycle_crd18_kf, main = "K-function for bicycle accidents 2018 points")
```
...
# Compute K-function using Kest(), with edge correction.
db18_02_kf <- spatstat.explore::Kest(db18_02_ppp, correction = "Ripley")

# When plotting the result of K-, L-, G-functions, spatstat uses an S3 method plot.fv()
# that overwrites the basic plot() function and specializes it.
plot(db18_02_kf, main = "K-function for bicycle accidents 2018 points")
```
...
# Compute K-function using Kest(), with edge correction.
bicycle_crd_kcross <- spatstat.explore::Kcross(bicycle_marks_ppp,i = "2018", j = "2019", correction = "Ripley")

# When plotting the result of K-, L-, G-functions, spatstat uses an S3 method plot.fv()
# that overwrites the basic plot() function and specializes it.
plot(bicycle_crd_kcross, main = "Cross-K-function for all bicycle accidents points 2018 and 2019")
```

```{r}
#Cross-K-function
# Compute K-function using Kest(), with edge correction.
bicycle_crd_kcross <- spatstat.explore::Kcross(bicycle_marks_ppp,i = "2018", j = "2021", correction = "Ripley")

# When plotting the result of K-, L-, G-functions, spatstat uses an S3 method plot.fv()
# that overwrites the basic plot() function and specializes it.
plot(bicycle_crd_kcross, main = "Cross-K-function for all bicycle accidents points 2018 and 2021")
```
...
# Find nearest neighbors within 20 kilometers (= 20000 m).
...
bicycle_accidentss_links <-
  spdep::nb2lines(bicycle_accidents_nb, coords = coords_2d, as_sf = TRUE)
...
Discuss your results. What did you find regarding the hot and cold spots in your acci- dent count layers? How do they compare to each other across layers and across (past) methods? etc. Discuss also the influence of the parameter settings (e.g., neighbor search distance, spatial weight formation) on your results.
