...
districts <- st_read("Zurich_city_districts_2024.gpkg")
```
...
Report the following numbers in table(s): a. Number of accidents by accident severity category b. Number of accidents by accident type c. Number of accidents involving pedestrians, bicycles, and motorcycles, respectively. And combinations thereof (pedestrian AND bicycle, pedestrian AND motorcycle etc.). Are there any accidents involving all three modes (pedestrian, bicycle, motorcycle)?
...
```{r}
#keep only X and Y coordinates for the further analysis, remove Z because it's 0 anyway
...
# We can somehow see a knee at about 900.
...
plot(bicycle_crd_2018, 
     ...
     main = "DBSCAN result with eps = 900",
     asp = 1)
...
plot(bicycle_crd_2018, 
     ...
     main = "DBSCAN result with eps = 400",
     asp = 1)
...
plot(bicycle_crd_2018, 
     ...
     main = "DBSCAN result with eps = 200",
     asp = 1)
```
...
# Now compute DBSCAN with different values for eps.
db19_01 <- dbscan::dbscan(bicycle_crd_2019, eps = 900, minPts = 3)
db19_01
...
# So, let's also try  400.
graphics::abline(h = 400, col = "blue")

# Now compute DBSCAN with different values for eps.
db20_01 <- dbscan::dbscan(bicycle_crd_2020, eps = 900, minPts = 3)
db20_01
...
db20_03 <- dbscan::dbscan(bicycle_crd_2020, eps = 200, minPts = 3)
db20_03
plot(bicycle_crd_2020, 
...
# Now compute DBSCAN with different values for eps.
db21_01 <- dbscan::dbscan(bicycle_crd_2021, eps = 900, minPts = 3)
db21_01
...
plot(bicycle_crd_2018, 
     cex = 0.5, pch = 19, col = db18_03$cluster + 1,
     main = "DBSCAN result eps = 200 (2018)",
     asp = 1)

# Plot for the year 2019
plot(bicycle_crd_2019, 
     cex = 0.5, pch = 19, col = db19_03$cluster + 1,
     main = "DBSCAN result eps = 200 (2019)",
     asp = 1)

# Plot for the year 2020
plot(bicycle_crd_2020, 
     cex = 0.5, pch = 19, col = db20_03$cluster + 1,
     main = "DBSCAN result eps = 200 (2020)",
     asp = 1)

# Plot for the year 2021
plot(bicycle_crd_2021, 
     cex = 0.5, pch = 19, col = db21_03$cluster + 1,
     main = "DBSCAN result eps = 200 (2021)",
     asp = 1)
...
```{r}
X_crd_18 <- as.numeric(bicycle_accidents_2018$AccidentLocation_CHLV95_E)
Y_crd_18 <- as.numeric(bicycle_accidents_2018$AccidentLocation_CHLV95_N)
win <- as.owin(st_bbox(bicycle_accidents_2018))
bicycle_crd18_ppp <- spatstat.geom::ppp(x = X_crd_18, y = Y_crd_18, window = win, unitname = c("meter", "meters"))
```
...
```{r}
# Cross-G-function 2018 and 2019
...
# When plotting the result of K-, L-, G-functions, spatstat uses an S3 method plot.fv()
# that overwrites the basic plot() function and specializes it.
plot(bicycle_crd_kcross, main = "Cross-G-function for all bicycle accidents points 2018 and 2019")
```
